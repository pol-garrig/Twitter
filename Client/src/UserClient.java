 ...
 */
public class UserClient implements MessageListener {
    ...
    private boolean listenTopic(String name) {
        ...
        try {
            consumer = session.createConsumer((Topic) context.lookup("dynamicTopics/" + name));
    ...
    public boolean followUser(String followedUser){
        System.out.println(user.getUsername()+ " wants to follow "+ followedUser);
        try {
